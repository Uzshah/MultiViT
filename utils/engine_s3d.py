...
def train_one_epoch(args, model, criterion1, criterion2, criterion3, optimizer, scheduler, train_loader, device):
    ...
    for i, batch in enumerate(train_loader):
        images = (batch["image"]*batch["mask"]).to(device)
        depth = (batch["depth"]*batch["depth_mask"]).to(device)
        semantic = (batch["semantic"]*batch["mask"]).to(device)
        normal = (batch["normal"]*batch["mask"]).to(device)
        albedo = (batch["albedo"]*batch["mask"]).to(device)
        shading = (batch["shading"]*batch["mask"]).to(device)
        ...
        loss1 = criterion1(outputs[0]*batch['depth_mask'].to(device), depth)
        loss2 = criterion2(outputs[1]*mask, semantic)
        # loss8 = criterion2(outputs[5], semantic)
        # loss3 = criterion2(outputs[-1], semantic)
        loss3 = criterion1(outputs[2]*mask, albedo)
        loss4 = criterion1(outputs[3]*mask, shading)
        loss7 = criterion1(outputs[4]*mask, normal)
        pred_rgb = outputs[3] * outputs[2]
        # rgb = albedo * shading
        rgb_gray, pred_rbg_gray = rgb_to_grayscale(images), rgb_to_grayscale(pred_rgb*mask)
        ...
        iou = mIoU(outputs[1]*mask, semantic, n_classes = args.num_classes)
        acc = pixel_accuracy(outputs[1]*mask, semantic)
...
def evalute(args, model, criterion1, criterion2,criterion3, valid_loader, device='cuda'):
    ...
    miou = 0.
    accuracy = 0.
    mre_, mae_, abs_, abs_rel_, sq_rel_ = 0., 0., 0., 0., 0.,
    rmse_, rmse_log_, log10_, a1_, a2_, a3_ = 0., 0., 0., 0., 0., 0.
    albedo_loss = 0.
    ...
    with torch.no_grad():
        for i, batch in enumerate(valid_loader):
            ...
            accuracy +=acc
            miou +=iou.item()
            mre, mae, abs, abs_rel, sq_rel, rmse, rmse_log, log10, a1, a2, a3 = compute_depth_metrics(depth, outputs[0]*batch['depth_mask'].to(device))
            mre_ += mre; mae_ += mae; abs_ +=abs; abs_rel_ +=abs_rel; sq_rel_ += sq_rel; 
            a1_ +=a1; a2_ +=a2; a3_ +=a3; rmse_ += rmse; rmse_log_ +=rmse_log; log10_ += log10;
    ...
    mre_ /= len(valid_loader); mae_ /= len(valid_loader); abs_ /= len(valid_loader); abs_rel_ /= len(valid_loader); 
    sq_rel_ /= len(valid_loader); rmse_ /= len(valid_loader); rmse_log_ /= len(valid_loader); log10_ /= len(valid_loader); 
    a1_ /= len(valid_loader); a2_ /= len(valid_loader); a3_ /= len(valid_loader);    
