...
from models.equisamplingpoint import genSamplingPattern
...
class Panoformer(nn.Module):
    def __init__(self, num_classes, target, img_size=256, in_chans=3, out_channel =1,
        ...
        if self.target=="all" or self.target == "shading":
            self.shading = OutputProj(in_channel=2 * embed_dim, out_channel=out_channel, kernel_size=3, stride=1,
                                      input_resolution=(img_size, img_size * 2))
        ...
        if self.target=="all" or self.target == "semantic":
            self.semantic = OutputProj(in_channel=2 * embed_dim, out_channel=num_classes, kernel_size=3, stride=1,
                                      input_resolution=(img_size, img_size * 2))
    ...
    def forward(self, x):
        ...
        dlayer1 = self.decoderlayer_3(deconv3)

        # Output Projection
        
        outputs = {}
        if self.target=="all" or self.target == "depth":
            outputs["pred_depth"] = self.output_proj(dlayer1)
        if self.target=="all" or self.target == "shading":
